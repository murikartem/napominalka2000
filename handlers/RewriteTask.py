...
async def fun_start(message: Message, state: FSMContext):
    ...
    text2 = []
    for task in all:
        text2.append(types.InlineKeyboardButton(text=f'{task[1]}, {task[2]}, {task[3]}', callback_data=f'num_{task[0]}'))
...
async def delete(callback: types.CallbackQuery, bot, state: FSMContext):
    num = int(callback.data.split('_')[1])
    cursor.execute(f'select * from zadacha where id = {num}')
    ...
    #time
        ...
        async def get_newtime(message: Message, state: FSMContext):
            ...
            cursor.execute(f'update zadacha set time=? where id=? ', (newtime, num))
            con.commit()
        ...
        async def day_rewrite(message: Message):
            cursor.execute(f'select day from zadacha where id = {num}')
        ...
        async def get_newday(message: Message, state: FSMContext):
            ...
            cursor.execute(f'update zadacha set day=? where id=? ', (newday, num))
            con.commit()
        ...
        async def task_rewrite(message: Message):
            cursor.execute(f'select task from zadacha where id = {num}')
        ...
        async def get_newtask(message: Message, state: FSMContext):
            ...
            cursor.execute(f'update zadacha set task=? where id=? ', (newtask, num))
            con.commit()
