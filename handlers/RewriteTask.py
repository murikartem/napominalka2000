...
async def fun_start(message: Message, state: FSMContext):
    ...
    await message.answer(text=f'какую задачу вы хотите отредоктировать?{all}', reply_markup=types.ReplyKeyboardRemove())
...
async def get_number(message: Message, state: FSMContext):
    ...
    for button in rw_task:
        builder.add(button)
    await message.answer(text=f'что иммено вы хотите отредоктировать?{data}', reply_markup=builder.as_markup(resize_keyboard=True))
    ...
    async def time_rewrite(message: Message):
        ...
        await message.answer(text=f'выберите время которое вы хотите вместо {data1[0]}', reply_markup=builder.as_markup(resize_keyboard=True))
    ...
    async def get_newtime(message: Message, state: FSMContext):
        ...
        print(newtime)
        cursor.execute(f'update zadacha set time={newtime} where id={number} ')
        con.commit()
    ...
    async def day_rewrite(message: Message):
        ...
        await message.answer(text=f'выберите день который вы хотите вместо {data2[0]}',
    ...
    async def get_newday(message: Message, state: FSMContext):
        ...
        print(newday)
        cursor.execute(f'update zadacha set day={newday} where id={number} ')
        con.commit()
    ...
    async def task_rewrite(message: Message):
        ...
        await message.answer(text=f'выберите задачу которую вы хотите вместо {data3[0]}',
                             reply_markup=builder.as_markup(resize_keyboard=True))
    ...
    async def get_newtask(message: Message, state: FSMContext):
        ...
        print(newtask)
        cursor.execute(f'update zadacha set task={newtask} where id={number} ')
        con.commit()
